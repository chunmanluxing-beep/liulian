...
## L11(2026-09-05):首屏视频改成「先压缩再入库」(本仓无产物改动,记录在此备查)

后台传高清视频时会直接甩出 GitHub 的英文报错。根因是**架构错了**:原先把分块收齐的
**未压缩原始视频**整体当作一个 GitHub blob 提交,而那个接口实测 30MB 能过、40MB 起就 422。

- 现在改成:分块收齐后**先在服务端用 ffmpeg 压成 1280 宽 / 前 20 秒 / 静音 / 无元数据的成品**
  (MP4 ≤4MB、WebM ≤3.5MB、封面 ≤200KB),**只把成品提交进私有仓**,原始文件不落库、
  不进任何 git 历史。实测单段可到 **296.6MB**(声明上限 300MB)。
- 本仓的 `scripts/build.py` 与首屏视频位(L5 的 `hero_media()`)**一行未动**;
  私有仓 `scripts/process.py` 新增 `adopt_hero()`,认出后台压好的成品就直接沿用、
  不再二次压缩,只在缺 WebM 时补一个。
- 另外把后台界面上一切**原始 API 报错回显**全部消灭:只显示提前写好的中文句子,
  真实技术细节只进服务器日志。

细节见私有仓 README 的 L11 章。

## L11b(2026-09-17):老图纳管 —— 本仓新增两件产物

业主反馈「后台看不到、也删不掉 Code 当初找来的示意图片」。真源改造在私有仓,
本仓只动 `scripts/build.py`,新增两件产物,**页面产物逐字节不变**(已实证)。

- **`photos/onsite.json`(新)**:`{板块: [id, …]}` —— ★官网页面上真正出现的那一批★。
  由 `picture()`(全站唯一产出 `<img>` 的函数)在渲染时如实收集,**不是另写一套规则**。
  后台据此显示「这张现在在不在官网上」,也是「后台清单 = 官网清单」这条等式的机器凭证。
  它与 `photos/index.json` 的区别:后者是**仓库里存着**的(当前 161 张),
  前者是**页面上看得见**的(当前 117 张)—— 差额来自三处既有规则:
  ① 某板块有客片后示意图整批退场;② 首屏放视频时 `shouping` 的图不上场;
  ③ 业务四类卡片每类只取前 3 张。
- **`CREDITS.md` 改为每次构建自动重建**:此前只在 `scripts/fetch_placeholders.py`
  里一次性生成,后台删图不会同步,署名台账会越积越旧。现在与 `credits.html` 同源于
  `photos/index.json`,删一张示意图 → 两处署名同时消失。
  ★口径差异是有意的★:`CREDITS.md` 记**仓库里存在**的全部示意图(分发即需署名),
  `credits.html` 记**页面上正在展示**的那一批。接手当天两者分别是 96 / 60 条。
- 等价性实证:改造后重跑 `build.py`,`index.html` / `en/index.html` / `credits.html` /
  `sitemap.xml` / `robots.txt` / `CREDITS.md` 与线上产物**逐字节一致**。
